...
function main()
    ...
    if(~exist('s','Var'))
        ...
        s = serial(serial_port,'BaudRate',4000000,'DataBits',8,'InputBufferSize',1600000); %20k is 6000 points * 8 byte each, so take 40k.
        fopen(s);     
    end
    ...
    % X1
    function M_1=X1(good_out,new_frame_1_idx,len_1_before)
        % disp)
                ...
                for i=1:len+1
                    ...
                    for j=1:size_A_sepi
                        if (A_sep{i}(3*(j-1)+1)==0)
                            ...
                            try
                                M_1(temp_row_1, j)=randi([1,10],1)*A_sep{i}(3*(j-1)+1)*80*(10-1) / ( A_sep{i}(3*(j-1)+2) ); % in
                            catch
                                disp('wasted M_1');
                            end % try
                        end
                ...
                for i=1:len+1
                    ...
                    for j=1:size_A_sepi
                        if (A_sep{i}(3*(j-1)+1)==0)
                            ...
                            % REMOVE 10, %% -> ACCURACY is no more, than 1/256, since M_! is uin8
                            try
                                M_2(temp_row_2, j)=randi([1,10],1)*A_sep{i}(3*(j-1)+1)*80*(10-1) / ( A_sep{i}(3*(j-1)+2) ); % in
                            catch
                                disp('wasted M_2');
                            end % try
                        end
                ...
                for i=1:len+1
                    ...
                    for j=1:size_A_sepi
                        if (A_sep{i}(3*(j-1)+1)==0)
                            ...
                            % REMOVE 10, %% -> ACCURACY is no more, than 1/256, since M_! is uin8
                            try
                                M_3(temp_row_3, j)=randi([1,10],1)*A_sep{i}(3*(j-1)+1)*80*(10-1) / ( A_sep{i}(3*(j-1)+2) ); % in
                            catch
                                disp('wasted M_3');
                            end % try
                        end
                ...
                for i=1:len+1
                    ...
                    for j=1:size_A_sepi
                        if (A_sep{i}(3*(j-1)+1)==0)
                            ...
                            % REMOVE 10, %% -> ACCURACY is no more, than 1/256, since M_! is uin8
                            try
                                M_4(temp_row_4, j)=randi([1,10],1)*A_sep{i}(3*(j-1)+1)*80*(10-1) / ( A_sep{i}(3*(j-1)+2) ); % in
                            catch
                                disp('wasted M_4');
                            end % try
                        end
                ...
                for i=1:len+1
                    ...
                    for j=1:size_A_sepi
                        if (A_sep{i}(3*(j-1)+1)==0)
                            ...
                            % REMOVE 10, %% -> ACCURACY is no more, than 1/256, since M_! is uin8
                            try
                                M_5(temp_row_5, j)=randi([1,10],1)*A_sep{i}(3*(j-1)+1)*80*(10-1) / ( A_sep{i}(3*(j-1)+2) ); % in
                            catch
                                disp('wasted M_5');
                            end % try
                        end
                ...
                for i=1:len+1
                    ...
                    for j=1:size_A_sepi
                        if (A_sep{i}(3*(j-1)+1)==0)
                            ...
                            % REMOVE 10, %% -> ACCURACY is no more, than 1/256, since M_! is uin8
                            try
                                M_6(temp_row_6, j)=randi([1,10],1)*A_sep{i}(3*(j-1)+1)*80*(10-1) / ( A_sep{i}(3*(j-1)+2) ); % in
                            catch
                                disp('wasted M_6');
                            end % try
                        end
