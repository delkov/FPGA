...
function main()
    ...
    size_to_read=500; % read when reached in buffer. the bigger -> the bigger chance to get 2 error -  even/odd elements after reading
            ...
            if (new_frame_2_idx == 0) % no new frame for sub_2
                ...
                if (len>0) % sometimes we can read only new frame, without new_line
                    ...
                    if (~frame_reversed_2) % we dont need to reverse frame..
                        ...
                        M_2_not_reversed_frame_first_line=M_2(temp_row_2-1,:);
                ...
                if(len==0) 
                    temp_row_2=2;
                    size_A_sepi=find(A_2==0,1,'first')-1;
                    A_sep{1}=A_2(1:size_A_sepi);
                    for j=1:size_A_sepi
                            if (A_2(3*(j-1)+2)==replace_what)
                                M_2(temp_row_2,j)=replace_to;
                            else
                                M_2(temp_row_2, j)=A_2(3*(j-1)+2)*720*0.15 /  A_2(3*(j-1)+3) ; % in
                            end     
                    end % for j
                    size_y=size(M_2_not_reversed_frame_last_line,2);
                    size_y_2=size(M_2_not_reversed_frame_first_line,2);
                    M_2(1,1:size_y)=M_2_not_reversed_frame_last_line;
                    M_2(2,1:size_y)=M_2_not_reversed_frame_first_line;
                ...
                % if (len==0)
                %     M_2_not_reversed_frame_last_line
