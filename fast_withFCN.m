...
function main()
    ...
    if(~exist('s','Var'))
        ...
        s.BytesAvailableFcn = {@READY_TO_READ,start};
    ...
    % all_out=textread('test_out.txt');
        ...
        function READY_TO_READ(obj,event,start)
    if toc(start) > 10
        return
    end
    ...
    left_out
        ...
        % if (size_to_read>read_trigger_size)
            out = [left_out; fread(s,size_to_read,'uint16')]; % both of them 16 bit -> it also 16bit; PREALLOCATED is not needed in such cases
