...
function main()
    ...
    if ~exist('h','var')   
        ...
        % set(gcf, 'Renderer','painter1s') 
        % set(h, 'GraphicsSmoothing','off') 
        % speed up FPS
        % set(gcf, 'GraphicsSmoothing','off')  % worled! use for 2017 (HG2)
        % set(gcf,'Renderer','OpenGL'); % probably not..
        set(gcf,'RendererMode','manual') % probably not..
       ...
       % F1
        ...
        % F2
        axes(ha(2));
        pl(2)=image(M_2,'CDataMapping','direct');
        if (~show_separated_lines)
            axis off;
        end
        ...
        % F3
        axes(ha(3));
        pl(3)=image(M_3,'CDataMapping','direct');%,[color(1) color(2)]);
        if (~show_separated_lines)
            axis off;
        end
...
% serial_port
    ...
    while 1  
        ...
        % completedIdx = fetchNext(F)
            ...
            if (redraw_2==true)% && redraw_3==true) 
                % disp('frame_2')
                counter=counter+1;
                redraw_2=false;
                redraw_3=false;
                ...
                drawnow %update%expose%update nocallbacks
                % delay_ms(11)
            end
