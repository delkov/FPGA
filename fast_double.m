...
function main()
    ...
    size_to_read=6000; % read when reached in buffer. 1k~2.7per frame for 300x4, 10 FPS. 11500 is working for 6-sub.. 6000 is minimum
    % number_of_zeros=5;
    ...
    function READY_TO_READ(~,~)
            ...
            % out = [left_out; fread(s,size_to_read,'uint16')]; 
            new = fread(s,size_to_read,'uint16');
            out = [left_out; new]; 
            
            % out(1:3)
            % out_idx = find(out==1488);
            % if (out_idx)
            %     disp('WRONG INTB')
            %     disp(out(out_idx-1))
            %     % out(out_idx)
